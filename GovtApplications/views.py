...
#     """Officer Logout"""
#     if request.method == "POST":
#         # Save user session data before clearing officer data
#         user_auth_id = request.session.get('_auth_user_id')
#         user_backend = request.session.get('_auth_user_backend')
#         user_hash = request.session.get('_auth_user_hash')
        
#         # Save any other user-specific session data you want to preserve
#         user_district = request.session.get('user_district')  # Example
        
#         # Delete only officer-related session keys
#         officer_keys_to_delete = []
#         for key in list(request.session.keys()):
#             if key.startswith('officer_'):
#                 officer_keys_to_delete.append(key)
        
#         for key in officer_keys_to_delete:
#             del request.session[key]
        
#         # If you're using default Django auth for officers, remove these too
#         # (Only if officer uses the default auth keys)
#         # if 'officer_auth_id' not in request.session:  # If officer uses default keys
...
#     # Clear officer keys
#     officer_keys_to_delete = [key for key in request.session.keys() if key.startswith('officer_')]
#     for key in officer_keys_to_delete:
#         del request.session[key]
